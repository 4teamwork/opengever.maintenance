"""
Adds a new group to the local roles of a specific object and reindexes it
in a transaction safe way. This allows us to update a huge number of local roles
during office hours

    bin/instance run src/opengever.maintenance/opengever/maintenance/scripts/reindex_allowed_roles_and_users.py --add-group MY_DUMMY_GROUP --collect --reindex

The script can also be used to just collect or reindex the local roles.
...
class Reindexer(object):
    ...
    def __init__(self, query, dry_run=True):
        ...
        self.catalog = api.portal.get_tool('portal_catalog')
        self.query = query
        self.brains = self.catalog.unrestrictedSearchResults(query)

    def reindex(self):
        Processor().run(self.brains, dry_run=self.dry_run, process_item_method=self.process_item)

    def process_item(self, brain):
        obj = brain.getObject()
        reindex_object_security_without_children(obj)
...
if __name__ == "__main__":
    ...
    if not options.add_group and not options.collect and not options.reindex:
    ...
    if options.reindex:
        reindexer = Reindexer(
            query={"UID": data_persistor.json_load()},
