...
class MeetingsContentMigrator(object):
    ...
    def migrate_agendaitems_to_subdossiers(self, meeting, meeting_dossier):
        ...
        for agendaitem in ProgressLogger(message, meeting.agenda_items, logger):
            ...
            if document:
                if agendaitem.has_proposal:
                    # document is in the submitted proposal, copy it
                    self.copy_and_add_to_mapping(document, dossier, meeting_dossier)
                else:
                    # document is in the meeting dossier, move it
