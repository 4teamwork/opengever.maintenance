...
Script to find field values that haven't been persisted on objects.
    ...
    bin/instance run find_fields_with_non_persisted_default_values.py
...
class NonPersistedValueFinder(object):
    ...
    def fields_with_default(self, schema):
        fields = []
        for name, field in getFieldsInOrder(schema):
            if field.__dict__.get('defaultFactory', field.__dict__.get('default')) is None:
                continue
            fields.append(field)
        return fields
    ...
    def check_for_missing_fields(self, obj):
        ...
        for schema in schemas:
            if schema.__identifier__ not in self.FIELD_CACHE:
                self.FIELD_CACHE[schema.__identifier__] = self.fields_with_default(schema)
