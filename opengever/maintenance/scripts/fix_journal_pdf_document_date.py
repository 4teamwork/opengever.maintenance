...
class JournalPDFDateFixer(object):

    def __init__(self):
        self.table = TextTable()
        self.table.add_row(["Document path", "Document title", "Date", "Interface"])

    def fix_journal_pdf_dates(self):
        for document in self.get_problematic_journal_pdfs():
            self.fix_document_date(document)

        print("Table of fixed documents")
        print(self.table.generate_output())
        print("\nSummary:")
        print("fixed {} documents".format(self.table.nrows))

    def fix_document_date(self, document):
        ...
        to_index = []
        row = [document.absolute_url_path(), document.title]
        ...
        if document_changed.date() > dossier_end_date and not IAutoGeneratedDocument.providedBy(document):
    ...
    def get_problematic_journal_pdfs(self):
        ...
        for dossier_brain in dossier_brains:
            ...
            # as well as documents that do not have the the expected title of a
            # dossier journal.
...
def main():
    ...
    date_fixer = JournalPDFDateFixer()
    date_fixer.fix_journal_pdf_dates()

    log_filename = LogFilePathFinder().get_logfile_path(
        'fix_journal_pdf_document_date', extension="csv")
    with open(log_filename, "w") as logfile:
        date_fixer.table.write_csv(logfile)
