...
class FilingNumberHelper(object):
    ...
    def is_valid_fn_number(self, filing_number):
        ...
        if not parts[2].isdigit() or len(parts[2]) != 4:
            return False
        # counter (ignore points, thats the delimiter for subdossiers)
        if not parts[3].replace(".", "").isdigit():
            return False
        return True
...
class FilingNumberChecker(Checker, FilingNumberHelper, ExcelReportMixin):
    ...
    def check_for_invalid_filing_numbers(self):
        """Check for invalid filing numbers"""

        dossiers_with_invalid_filing_number = []
        fns_and_paths = self.get_filing_numbers()
        for fn, path in fns_and_paths:
            if not self.is_valid_fn_number(fn):
                dossiers_with_invalid_filing_number.append((fn, path))
        return dossiers_with_invalid_filing_number
